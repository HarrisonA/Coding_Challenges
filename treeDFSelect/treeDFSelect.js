...
Tree.prototype.DFSelect = function(filter) {
  ...
  var recursionTree = function (node){
    if(filter(node.value)){
      results.push(node.value)
    }
    for (var i=0; i<node.children.length; i++){
      recursionTree(node.children[i]);
    }
...
var greaterThanFour = function (value) {
  return (value > 4);
}
...
root1.DFSelect(greaterThanFour);
root1.DFSelect(oddNumber);
root1.DFSelect(evenNumber);

  ...
  for (var i=0; i < arr1.length; i++) {
    for (var j=0; j < arr2.length; j++) {
      if(arr1[i] > arr2[j]){
        ...
        // because the j index will increase on the next iteration BUT the array has 
        // shifted left (Example: Whatever was at j[3] is now at j[2])
        j--;
        ...
        continue;
          ...
          // console.log("\ni an j are the same, add both: ", arr1[i], arr1[j]);
          tempArr.push(arr1[i]);
          tempArr.push(arr2[j]); 
          arr1.splice(i,1);
          arr2.splice(j,1);
        ...
        // console.log("\ncurrent i: ", arr1[i], "current j: ", arr2[j]);
        // console.log("i is less than j, so add: ", arr1[i]);
        tempArr.push(arr1[i]);
        arr1.splice(i,1);
        i--;
        break;
    ...
    // after looping through all elements put the arr[i] into the final array
    // tempArr.push(arr1[i]);
  }
  // if any elements are left in arr2, then add them to the final array
  if (arr2.length>0){
    // console.log("\nremaining elements in arr2: ", arr2);
    tempArr = tempArr.concat(arr2);
    // console.log("after the arr2 concat",tempArr);
  }
...
var mergeSort = function(array) {
  ...
  while (resultArr.length>1){
    danger++;  if (danger > 20) return;

    // if odd number of elements, then combine the last two elements
...
mergeSort([5,2,3,100, -1 ,3,4,5, 12, 22,33, 12, -5, 10])
